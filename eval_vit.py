...
model_class_dict = {'pt_vgg': torch_models.vgg16_bn,
                    ...
                    'pt_inc':  torch_models.inception_v3
                    }
...
def sense_mask(model, x_input):
    ...
    cam = EigenGradCAM(model=model.model, target_layers=target_layer, use_cuda=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default='pt_inc', type=str)
    parser.add_argument('--n_ex', type=int, default=960)
    ...
    if args.attack in ['rs_attack']:
        ...
        with torch.no_grad():
            ...
            # run the attack
            pred_adv = pred.clone()
            pred_adv_1 = pred.clone()
            pred_adv_2 = pred.clone()
            pred_adv_3 = pred.clone()
            for counter in range(n_batches):
                ...
                with torch.enable_grad():
                    print("gening")
                    model_r = PretrainedModel('pt_vgg')
                    assert not model_r.model.training
                    print(model_r.model.training)
                    cam_mask = section_mask(model_r, x_curr)

                qr_curr, adv, adv_300, adv_700, adv_1000 = adversary.perturb(x_curr, cam_mask, y_curr)
                ...
                # output_1 = model(adv_300.cuda())
                # output_2 = model(adv_700.cuda())
                # output_3 = model(adv_1000.cuda())
                output = model(adv.cuda())
                output_1 = model(adv_300.cuda())
                output_2 = model(adv_700.cuda())
                output_3 = model(adv_1000.cuda())
                if not args.targeted:
                    ...
                    acc_curr_1 = (output_1.max(1)[1] == y_curr).float().cpu()
                    acc_curr_2 = (output_2.max(1)[1] == y_curr).float().cpu()
                    acc_curr_3 = (output_3.max(1)[1] == y_curr).float().cpu()

                else:
                    acc_curr = (output.max(1)[1] != y_curr).float().cpu()
                pred_adv[ind_to_fool[counter * bs:(counter + 1) * bs]] = acc_curr.clone()
                pred_adv_1[ind_to_fool[counter * bs:(counter + 1) * bs]] = acc_curr_1.clone()
                pred_adv_2[ind_to_fool[counter * bs:(counter + 1) * bs]] = acc_curr_2.clone()
                pred_adv_3[ind_to_fool[counter * bs:(counter + 1) * bs]] = acc_curr_3.clone()
            ...
            print('success rate={:.0f}/{:.0f} ({:.2%}) \n'.format(
                pred.sum() - pred_adv_1.sum(), pred.sum(), (pred.sum() - pred_adv_1.sum()).float() / pred.sum()) )
            print("log2:")
            print('success rate={:.0f}/{:.0f} ({:.2%}) \n'.format(
                pred.sum() - pred_adv_2.sum(), pred.sum(), (pred.sum() - pred_adv_2.sum()).float() / pred.sum()))
            print("log3:")
            print('success rate={:.0f}/{:.0f} ({:.2%}) \n'.format(
                pred.sum() - pred_adv_3.sum(), pred.sum(), (pred.sum() - pred_adv_3.sum()).float() / pred.sum()))
